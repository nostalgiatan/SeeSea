...
use seesea_core::derive::types::{EngineType, ResultType, SearchQuery, SearchResultItem};
...
fn test_result_cache_fulltext_search() {
    ...
    let items1 = vec![
        ...
        sample_result_item(
            ...
            "Official Rust documentation and tutorials",
        ),
    ];
    ...
    // 缓存结果
    result_cache
        .set(&query1, "TestEngine", &result1, None)
        .expect("缓存失败");
    result_cache
        .set(&query2, "TestEngine", &result2, None)
        .expect("缓存失败");

    // 测试全文搜索 - 搜索包含 "programming" 的结果
    let keywords = vec!["programming".to_string()];
    let search_results = result_cache
        .search_fulltext(&keywords, false, None)
        .expect("全文搜索失败");
    ...
    // 测试限制结果数量
    let keywords = vec!["programming".to_string()];
    let limited_results = result_cache
        .search_fulltext(&keywords, false, Some(1))
        .expect("全文搜索失败");
...
fn test_result_cache_search_with_stale() {
    ...
    let items = vec![sample_result_item(
        "Test Result",
        "https://example.com",
    ...
    // 设置1秒过期
    result_cache
        .set(&query, "TestEngine", &result, Some(Duration::from_secs(1)))
        .expect("缓存失败");

    // 立即搜索 - 应该找到结果（不包含过期的）
    let keywords = vec!["test".to_string()];
    let fresh_results = result_cache
        .search_fulltext(&keywords, false, None)
        .expect("搜索失败");
    ...
    // 搜索 - 不包含过期结果
    let no_stale_results = result_cache
        .search_fulltext(&keywords, false, None)
        .expect("搜索失败");
    assert_eq!(no_stale_results.len(), 0, "不应该找到过期的结果");

    // 搜索 - 包含过期结果
    let with_stale_results = result_cache
        .search_fulltext(&keywords, true, None)
        .expect("搜索失败");
...
fn test_rss_cache_fulltext_search() {
    ...
    // 缓存 RSS feed
    rss_cache
        .set("https://example.com/rss", &feed, false, None, None)
        .expect("缓存 RSS feed 失败");

    // 测试全文搜索 - 搜索包含 "rust" 的 RSS 项
    let keywords = vec!["rust".to_string()];
    let search_results = rss_cache
        .search_fulltext(&keywords, false, None)
        .expect("RSS 全文搜索失败");
    ...
    for (feed_url, item) in &search_results {
        ...
        let contains_keyword = item.title.to_lowercase().contains("rust")
            ...
            || item
                .content
                .as_ref()
                .map(|c| c.to_lowercase().contains("rust"))
                .unwrap_or(false);
    ...
    assert!(
        !prog_results.is_empty(),
        "应该找到至少一个包含 'programming' 的 RSS 项"
    );
}
...
fn test_rss_cache_list_all_feeds() {
    ...
    rss_cache
        .set("https://example1.com/rss", &feed1, false, None, None)
        .expect("缓存 feed 1 失败");
    rss_cache
        .set("https://example2.com/rss", &feed2, false, None, None)
        .expect("缓存 feed 2 失败");
...
fn test_multiple_keyword_search() {
    ...
    result_cache
        .set(&query, "TestEngine", &result, None)
        .expect("缓存失败");
    ...
    let rust_results = result_cache
        .search_fulltext(&rust_only, false, None)
        .expect("搜索失败");
