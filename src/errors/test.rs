...
mod tests {
    ...
    fn test_search_errors() {
        ...
        // 测试零结果错误
        let error = search::zero_results("bing");
        assert_eq!(error.code(), search::ZERO_RESULTS);
        assert_eq!(error.severity(), ErrorSeverity::Warning);

        // 测试向后兼容的search_error函数
        let error = search::search_error("测试搜索错误");
        assert_eq!(error.code(), search::SEARCH_ERROR_BASE);
    }
    ...
    fn test_error_categories() {
        ...
        assert_eq!(search::ENGINE_UNAVAILABLE / 1000, 2); // 搜索错误码范围
