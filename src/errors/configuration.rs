...
pub fn config_parse_failed(path: &str, reason: &str) -> ErrorInfo {
    ErrorInfo::new(CONFIG_PARSE_FAILED, format!("配置文件 '{path}' 解析失败: {reason}"))
        .with_category(ErrorCategory::Configuration)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn invalid_config_value(key: &str, value: &str) -> ErrorInfo {
    ErrorInfo::new(INVALID_CONFIG_VALUE, format!("配置项 '{key}' 值 '{value}' 无效"))
        .with_category(ErrorCategory::Configuration)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn config_type_error(key: &str, expected_type: &str, actual_type: &str) -> ErrorInfo {
    ErrorInfo::new(CONFIG_TYPE_ERROR, format!("配置项 '{key}' 类型错误: 期望 {expected_type}, 实际 {actual_type}"))
        .with_category(ErrorCategory::Configuration)
        .with_severity(ErrorSeverity::Error)
}
...
/// # 返回
/// 包含配置项值超出范围信息的错误对象
pub fn config_value_out_of_range<T: std::fmt::Display>(key: &str, value: T, min: T, max: T) -> ErrorInfo {
    ErrorInfo::new(CONFIG_VALUE_OUT_OF_RANGE, format!("配置项 '{key}' 值 {value} 超出范围 [{min}, {max}]"))
        .with_category(ErrorCategory::Configuration)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn config_version_mismatch(expected_version: &str, actual_version: &str) -> ErrorInfo {
    ErrorInfo::new(CONFIG_VERSION_MISMATCH, format!("配置版本不匹配: 期望 {expected_version}, 实际 {actual_version}"))
        .with_category(ErrorCategory::Configuration)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn config_permission_error(path: &str, operation: &str) -> ErrorInfo {
    ErrorInfo::new(CONFIG_PERMISSION_ERROR, format!("{operation} 配置文件 '{path}' 权限被拒绝"))
        .with_category(ErrorCategory::Configuration)
        .with_severity(ErrorSeverity::Error)
}
