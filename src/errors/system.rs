...
pub fn system_call_failed(syscall: &str, reason: &str) -> ErrorInfo {
    ErrorInfo::new(SYSTEM_CALL_FAILED, format!("系统调用 '{syscall}' 失败: {reason}"))
        .with_category(ErrorCategory::System)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn version_incompatible(expected: &str, actual: &str) -> ErrorInfo {
    ErrorInfo::new(VERSION_INCOMPATIBLE, format!("系统版本不兼容: 期望 {expected}, 实际 {actual}"))
        .with_category(ErrorCategory::System)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn permission_error(operation: &str, resource: &str) -> ErrorInfo {
    ErrorInfo::new(PERMISSION_ERROR, format!("系统权限错误: 无法对 '{resource}' 执行 '{operation}' 操作"))
        .with_category(ErrorCategory::System)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn system_overload(metric: &str, value: f64, threshold: f64) -> ErrorInfo {
    ErrorInfo::new(SYSTEM_OVERLOAD, format!("系统过载: {metric} 达到 {value:.2}, 超过阈值 {threshold:.2}"))
        .with_category(ErrorCategory::System)
        .with_severity(ErrorSeverity::Warning)
}
