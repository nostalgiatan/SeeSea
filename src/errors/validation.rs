...
pub fn field_too_short(field: &str, min_length: usize, actual_length: usize) -> ErrorInfo {
    ErrorInfo::new(FIELD_TOO_SHORT, format!("字段 '{field}' 太短: 要求至少 {min_length} 个字符, 实际 {actual_length} 个字符"))
        .with_category(ErrorCategory::Validation)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn field_too_long(field: &str, max_length: usize, actual_length: usize) -> ErrorInfo {
    ErrorInfo::new(FIELD_TOO_LONG, format!("字段 '{field}' 太长: 要求最多 {max_length} 个字符, 实际 {actual_length} 个字符"))
        .with_category(ErrorCategory::Validation)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn invalid_number(value: &str, field: &str) -> ErrorInfo {
    ErrorInfo::new(INVALID_NUMBER, format!("字段 '{field}' 不是有效的数字: {value}"))
        .with_category(ErrorCategory::Validation)
        .with_severity(ErrorSeverity::Error)
}
...
/// # 返回
/// 包含无效枚举值信息的错误对象
pub fn invalid_enum_value(value: &str, field: &str, allowed_values: &[&str]) -> ErrorInfo {
    let allowed = allowed_values.join(", ");
    ErrorInfo::new(INVALID_ENUM_VALUE, format!("字段 '{field}' 值 '{value}' 无效: 允许的值为 [{allowed}]"))
        .with_category(ErrorCategory::Validation)
        .with_severity(ErrorSeverity::Error)
}
...
/// # 返回
/// 包含重复值信息的错误对象
pub fn duplicate_value(value: &str, field: &str) -> ErrorInfo {
    ErrorInfo::new(DUPLICATE_VALUE, format!("字段 '{field}' 值 '{value}' 已存在"))
        .with_category(ErrorCategory::Validation)
        .with_severity(ErrorSeverity::Error)
}
