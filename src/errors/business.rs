...
pub fn resource_not_found(resource_type: &str, resource_id: &str) -> ErrorInfo {
    ErrorInfo::new(RESOURCE_NOT_FOUND, format!("{resource_type} '{resource_id}' 不存在"))
...
pub fn business_rule_violation(rule: &str, reason: &str) -> ErrorInfo {
    ErrorInfo::new(BUSINESS_RULE_VIOLATION, format!("违反业务规则 '{rule}': {reason}"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn invalid_state(current_state: &str, expected_state: &str, operation: &str) -> ErrorInfo {
    ErrorInfo::new(INVALID_STATE, format!("无效的状态 '{current_state}'，无法执行操作 '{operation}'，期望状态为 '{expected_state}'"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn operation_not_allowed(operation: &str, reason: &str) -> ErrorInfo {
    ErrorInfo::new(OPERATION_NOT_ALLOWED, format!("操作 '{operation}' 不允许: {reason}"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn quota_exceeded(quota_type: &str, limit: u64, usage: u64) -> ErrorInfo {
    ErrorInfo::new(QUOTA_EXCEEDED, format!("{quota_type} 配额超出: 使用 {usage}，限制 {limit}"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn rate_limit_exceeded(resource: &str, limit: u64, window: u64) -> ErrorInfo {
    ErrorInfo::new(RATE_LIMIT_EXCEEDED, format!("{resource} 速率限制超出: {limit} 次/{window} 秒"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn dependency_failed(dependency: &str, reason: &str) -> ErrorInfo {
    ErrorInfo::new(DEPENDENCY_FAILED, format!("依赖 '{dependency}' 失败: {reason}"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
...
pub fn concurrency_error(operation: &str, resource: &str) -> ErrorInfo {
    ErrorInfo::new(CONCURRENCY_ERROR, format!("并发操作 '{operation}' 失败，资源 '{resource}' 已被修改"))
        .with_category(ErrorCategory::Business)
        .with_severity(ErrorSeverity::Error)
}
