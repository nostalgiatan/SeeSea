...
/// 精确匹配加分
fn exact_match_bonus(text: &str, query: &str) -> f64 {
...
/// URL 相关性评分
fn url_relevance(url: &str, query: &str) -> f64 {
...
/// 位置评分（原始搜索引擎排名）
pub fn position_score(position: usize) -> f64 {
    // 对数衰减：前几个结果分数明显更高
    // position 从 0 开始
    1.0 / (1.0 + (position as f64).ln())
}
...
mod tests {
    ...
    fn test_exact_match() {
        ...
        assert_eq!(exact_match_bonus("rust programming language", "rust"), 0.5);
