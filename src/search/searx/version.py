...
def get_git_url_and_branch():
    ...
    try:
        ...
        try:
            ...
            return "https://github.com/searxng/searxng", "master"
...
def get_information() -> tuple[str, str, str, str, str]:
    ...
    try:
        ...
        logger.debug("Error while getting the version: %s", getattr(ex, 'stderr', str(ex)))
        version_string, version_tag, docker_tag = "1.0.0", "1.0.0", "1.0.0"
    try:
        git_url, git_branch = get_git_url_and_branch()
    except Exception as ex:
        logger.debug("Error while getting the git URL & branch: %s", getattr(ex, 'stderr', str(ex)))
        git_url, git_branch = "https://github.com/searxng/searxng", "master"
