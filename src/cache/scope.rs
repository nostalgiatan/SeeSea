...
mod tests {
    ...
    fn test_scope_cache() {
        ...
        scope_cache.set(key.to_string(), value.clone(), None).expect("设置缓存失败");
    ...
    fn test_scope_cache_expiration() {
        let temp_dir = std::env::temp_dir();
        let db_path = temp_dir.join(format!("test_scope_cache_expiration_{}", std::process::id()));
        ...
        scope_cache.set(key.to_string(), value, Some(Duration::from_secs(1))).expect("设置缓存失败");
        ...
        let result = scope_cache.get_include_stale(key).expect("获取过期缓存失败");
        assert!(result.is_some());
