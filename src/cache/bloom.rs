...
impl<T> BloomFilter<T> {
    ...
    fn calculate_parameters(expected_elements: u64, false_positive_rate: f64) -> (usize, usize) {
        ...
        let m = (- (expected_elements as f64) * false_positive_rate.ln() / (2.0_f64.ln().powi(2))).ceil() as usize;
    ...
    /// 
    /// 返回哈希值
    fn hash(&self, item: &T) -> u64 where T: std::hash::Hash {
    ...
    /// 
    /// 返回多个哈希位置
    fn get_positions(&self, item: &T) -> Vec<usize> where T: std::hash::Hash {
        let mut positions = Vec::with_capacity(self.num_hashes);
        let hash = self.hash(item);
        
        // 使用双重哈希技术生成多个哈希值
        for i in 0..self.num_hashes {
            let position = (hash.wrapping_add((i as u64).wrapping_mul(hash.wrapping_add(i as u64)))) % self.bits.len() as u64;
    ...
    /// 
    /// * `item` - 要添加的元素
    pub fn add(&mut self, item: &T) where T: std::hash::Hash {
    ...
    pub fn contains(&self, item: &T) -> bool where T: std::hash::Hash {
        let positions = self.get_positions(item);
