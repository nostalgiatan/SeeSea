...
use crate::errors::{Result, business_error, connection_failed, database_error};
use crate::sys::{ComponentConfig, ComponentId, ComponentType, SystemController};
use crate::vector_store::types::{CacheConfig, DynamicAdjustmentConfig};
use crate::vector_store::{Document, VectorStore, VectorStoreConfig, VectorStoreResult};
...
impl QdrantVectorStore {
    ...
    pub async fn new(config: VectorStoreConfig) -> Result<Arc<Self>> {
        ...
        // Determine distance metric using the correct Distance enum
        let distance = match config.distance.as_deref().unwrap_or("Cosine") {
            ...
            client
                .create_collection(
                    CreateCollectionBuilder::new(&config.collection_name)
                        ...
                        .shard_number(config.shard_number.unwrap_or(1) as u32)
                        .replication_factor(config.replication_factor.unwrap_or(1) as u32),
                )
                .await
                .map_err(|e| {
        ...
        // Get dynamic adjustment configuration or use default
        let dynamic_adjustment = config.dynamic_adjustment.unwrap_or_default();

        // Initialize cache configuration and scope cache
        let cache_config = config.cache;
        let scope_cache = if let Some(ref cache_cfg) = cache_config {
            if cache_cfg.enabled {
                // Create cache interface and scope cache
                let cache_interface =
                    crate::cache::CacheInterface::new(crate::cache::CacheImplConfig {
                        db_path: ".seesea/cache.db".to_string(),
                        default_ttl_secs: cache_cfg.ttl_secs,
                        max_size_bytes: 1024 * 1024 * 1024, // 1GB default
                        enabled: true,
                        compression: false,
                        mode: crate::cache::CacheMode::HighThroughput,
                ...
                let scope_name = cache_cfg
                    .scope
                    .clone()
                    .unwrap_or(format!("vector_store.{}", config.collection_name));
    ...
    async fn update_batch_sizes(&self) -> Result<()> {
        ...
        if let Some(ref system_controller) = self.system_controller {
            ...
            let component_status = crate::sys::types::ComponentStatus {
...
Error: {}",
    ...
    async fn search(
        ...
        if let Some(ref cache) = self.scope_cache {
            ...
            let ttl = self
                .cache_config
                .as_ref()
                .map(|cfg| Duration::from_secs(cfg.ttl_secs))
                .or(Some(Duration::from_secs(3600))); // Default 1 hour
    ...
    async fn get(&self, id: &str) -> Result<Option<Document>> {
        ...
        if let Some(point) = points.result.into_iter().next() {
            ...
            let payload_json = serde_json::to_value(point.payload).map_err(|e| {
                crate::errors::database_error(format!("Failed to convert payload to JSON: {e}"))
            })?;
    ...
    async fn get_stats(&self) -> Result<crate::vector_store::types::VectorStoreStats> {
        ...
        Ok(crate::vector_store::types::VectorStoreStats {
            points_count: 0,
