...
use seesea_derive::{
    ...
    SearchQuery, SearchResult, SearchResultItem, ValidationError, engine::EngineHealth,
    types::AboutInfo,
};
...
impl SearchEngine for PythonEngineWrapper {
    ...
    async fn health_check(&self) -> Result<EngineHealth, Box<dyn std::error::Error + Send + Sync>> {
        Ok(EngineHealth {
    ...
    fn validate_query(&self, query: &SearchQuery) -> Result<(), ValidationError> {
        if query.query.is_empty() {
            return Err(ValidationError {
                ...
                message: "Query field cannot be empty".to_string(),
