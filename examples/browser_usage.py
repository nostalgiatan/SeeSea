...
async def example_direct_engine_usage():
    ...
    config = BrowserConfig(headless=True, stealth=True, timeout=30000)
...
async def example_callback_for_rust():
    ...
    params = {"query": "新华网", "page": 1, "page_size": 5, "category": "0"}
    ...
    async with XinhuaEngine(config) as engine:
        try:
            ...
            results = await engine.search_with_retry(
                query="区块链", page=1, max_results=5, max_retries=3, retry_delay=1.0
            )
